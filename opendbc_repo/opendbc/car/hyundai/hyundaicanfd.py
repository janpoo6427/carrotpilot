...
def create_acc_control_scc2(packer, CAN, enabled, accel_last, accel, stopping, gas_override, set_speed, hud_control, hyundai_jerk, CS):
  ...
  _tf = float(hud_control.tFollow) if hud_control.tFollow > 0.1 else 1.45
  values["TARGET_DISTANCE"] = int(CS.out.vEgo * _tf + 6.0)
  ...
  if CP.flags & HyundaiFlags.CAMERA_SCC.value:
    ...
    if frame % 5 == 0:
      ...
      if CS.adrv_0x161 is not None:
        ...
        _tf = float(hud_control.tFollow) if hud_control.tFollow > 0.1 else 1.45
        values["TARGET_DISTANCE"] = int(CS.out.vEgo * _tf + 6.0) if hud_control.lead_visible else 0
