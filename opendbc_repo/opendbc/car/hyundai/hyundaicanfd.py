...
def create_acc_control_scc2(packer, CAN, enabled, accel_last, accel, stopping, gas_override, set_speed, hud_control, hyundai_jerk, CS, lead_visible: bool = False):
...
def create_ccnc_messages(CP, packer, CAN, frame, CC, CS, hud_control,
                         ...
                         enable_corner_radar, stopping, canfd_debug, lead_visible: bool = False):
  ret = []
  ...
  if CP.flags & HyundaiFlags.CAMERA_SCC.value:
    ...
    if frame % 5 == 0:
      ...
      if CS.ccnc_0x162 is not None:
        ...
        if lead_visible and hud_control.leadDistance > 0:
