...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.flags & HyundaiFlags.CANFD:
      ...
      if camera_scc:
        ...
        can_sends.extend(hyundaicanfd.create_steering_messages(self.packer, self.CP, self.CAN, CC.latEnabled, apply_steer_req, apply_torque, apply_angle, self.lkas_max_torque, angle_control))
  ...
  def canfd_toggle_adas(self, CC, CS):
    ...
    if self.MainMode_ACC_trigger == trigger_min and self.LFA_trigger == trigger_min:
      auto_main_allowed = CS.auto_engage_mode  in (2, 3) and not CS.manual_main_off_latched
      if CC.enabled and not CS.MainMode_ACC and CS.out.vEgo > 3. and auto_main_allowed:
