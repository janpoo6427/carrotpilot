...
class CarController(CarControllerBase):
  def __init__(self, dbc_names, CP):
    ...
    self.lead_visible_counter = 0
    self.lead_visible_hys = False
    ...
    if self.CP.flags & HyundaiFlags.CANFD:
      ...
      if self.CP.openpilotLongitudinalControl:
        ...
        if True: #not camera_scc:
          can_sends.extend(hyundaicanfd.create_ccnc_messages(self.CP, self.packer, self.CAN, self.frame, CC, CS, hud_control, apply_angle, left_lane_warning, right_lane_warning, self.enable_corner_radar, stopping, self.canfd_debug, self.lead_visible_hys))
          if hda2:
        ...
        if self.frame % 2 == 0:
          if self.CP.flags & HyundaiFlags.CAMERA_SCC.value:
            msg = hyundaicanfd.create_acc_control_scc2(self.packer, self.CAN, CC.enabled, self.accel_last, accel, stopping, CC.cruiseControl.override,
                                                             set_speed_in_units, hud_control, self.hyundai_jerk, CS, self.lead_visible_hys)
            if msg is not None:
